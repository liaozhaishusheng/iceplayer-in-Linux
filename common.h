...
#define dialog(X) {GtkWidget *dialog = gtk_message_dialog_new(NULL,GTK_DIALOG_DESTROY_WITH_PARENT    ,GTK_MESSAGE_QUESTION,GTK_BUTTONS_OK,_(X));gtk_dialog_run((GtkDialog *)dialog);gtk_widget_destroy(dialog);}
#define popup_menu(X,Y,Z) {gtk_menu_popup (GTK_MENU(X),NULL,NULL,NULL,NULL,3,gtk_get_current_event_time());gint x,y;gint w,h;gint sx,sy; gtk_window_get_size ( (GtkWindow *)gtk_widget_get_toplevel((GtkWidget *)X),&w,&h); gdk_window_get_pointer( NULL,&x,&y,NULL); gtk_window_get_position( (GtkWindow *)gtk_widget_get_toplevel((GtkWidget *)X),&sx,&sy);    if (h>=gdk_screen_height()-y)        gtk_window_move((GtkWindow *)gtk_widget_get_toplevel((GtkWidget *)X),sx,sy-Z);    else    gtk_window_move((GtkWindow *)gtk_widget_get_toplevel((GtkWidget *)X),sx,sy+Z);    if (w>=gdk_screen_width()-x)        gtk_window_move((GtkWindow *)gtk_widget_get_toplevel((GtkWidget *)X),sx-Y,sy);    else        gtk_window_move((GtkWindow *)gtk_widget_get_toplevel((GtkWidget *)X),sx+Y,sy);}
#define CONFIG_READ_FAILED if(error!=NULL) {print_error("读取配置出错！%s\n",error->message);error=NULL;}
#define SKIN_CONFIG_READ_FAILED if(error!=NULL) {print_error("读取皮肤配置出错！%s\n",error->message);return FALSE;}
