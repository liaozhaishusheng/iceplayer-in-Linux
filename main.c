...
void init_configure_to_iceplayer_after_list() {
    print_debug("初始化 some configures");

    if (!configure.mini) gtk_widget_hide(iceplayer.mini_window);
    else gtk_window_move(iceplayer.mini_window,configure.mini_x,configure.mini_y);
...
void iceplayer_init(int argc, char *argv[]) {
    ...
    bindtextdomain (GETTEXT_PACKAGE, "/usr/share/iceplayer/locale");
