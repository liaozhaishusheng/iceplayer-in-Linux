...
void on_downloader_button_lrc123_enter (GtkButton *button,GtkWidget *iceplayer_lrc_downloader) {
    if (GTK_IS_WIDGET(iceplayer_lrc_downloader))gtk_widget_destroy(iceplayer_lrc_downloader);
    last_timer=-1;
    gtk_show_uri(NULL,"http://www.lrc123.com",NULL,NULL);
}
...
void download_lrc_by_num(gint num){
        ...
        g_timeout_add(100,download_lrc,hostname);
}
...
gboolean check_down(GtkWidget *widget) {
    ...
    if (GTK_IS_WIDGET(widget)) {
        gtk_button_set_label(widget,name);
        g_free_n(name);
    } else {
        last_timer=0;
        g_free_n(name);
            ...
            if(main_lrc_search->uri[0]==0){
                print_notify("iceplayer","歌词搜索无结果");
                 if(configure.edit_search_word) {//无结果并设定了在无结果的时候显示对话框
                    g_timeout_add(1000,check_down,iceplayer_lrc_downloader_button_download);
                    gtk_dialog_run(iceplayer_lrc_downloader);
                }
            }else{//有结果并设定了时间为0
                if(wait){//如果是手动按快捷键的话
                    g_timeout_add(1000,check_down,iceplayer_lrc_downloader_button_download);
...
void search_lrc() {
    ...
    if (download(hostname,filename,savepath,error)) {
        ...
        if (file) {
            ...
            g_timeout_add(50,choose_lrc_to_download,main_lrc_search);
            fclose(file);
        } else {
...
gboolean lrc_refresh() {
    ...
    if (configure.lrc) {
        ...
        if (cou==20) {
            ...
            if (GTK_IS_WIDGET(iceplayer.widget_lrc_label_hpaned))gtk_label_set_text(iceplayer.widget_lrc_label_hpaned,_("歌词功能关闭"));
            if (GTK_IS_WIDGET(iceplayer.widget_lrc_label))gtk_label_set_text(iceplayer.widget_lrc_label,_("歌词功能关闭"));
            if (GTK_IS_WIDGET(widget_lrc.label))gtk_label_set_text(widget_lrc.label,_("歌词功能关闭"));
        }
    }
    return TRUE;
}
...
gchar *get_lrc_file_path_at_song_path(gchar song_uri[STRINGS_LENGTH]) {
    ...
    g_timeout_add(10,ref,song_lrc_path);
...
gchar *get_lrc_file_path_at_custom_path(gchar song_uri[STRINGS_LENGTH],gint type) {
    ...
    switch (type) {
    case 1: {
        if (gtk_entry_get_text(configure_widget.local_lrc_path_1_entry )!=NULL)
            strcpy(song_lrc_path,gtk_entry_get_text(configure_widget.local_lrc_path_1_entry ));
        break;
    }
    case 2: {
        if (gtk_entry_get_text(configure_widget.local_lrc_path_2_entry )!=NULL)
            strcpy(song_lrc_path,gtk_entry_get_text(configure_widget.local_lrc_path_2_entry ));
        break;
    }
    case 3: {
        if (gtk_entry_get_text(configure_widget.local_lrc_path_3_entry )!=NULL)
            strcpy(song_lrc_path,gtk_entry_get_text(configure_widget.local_lrc_path_3_entry ));
        break;
    }
    case 4: {
        if (gtk_entry_get_text(configure_widget.lrc_download_path_entry )!=NULL)
            strcpy(song_lrc_path,gtk_entry_get_text(configure_widget.lrc_download_path_entry ));
        break;
    }
    }
