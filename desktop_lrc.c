...
gboolean desktop_lrc_show() {
    ...
    gtk_window_resize(desktop_lrc.window,width,height);

    if ((!configure.desktop_lrc_lock&&desktop_lrc.show_lock_background)||(gtk_notebook_get_current_page(configure_widget.lrc_notebook)==2&&gtk_notebook_get_current_page(iceplayer.notebook)==SWITCH_CONFIG&&gtk_notebook_get_current_page(configure_widget.notebook)==3&&!configure.desktop_lrc_lock)) {
        //print_debug("画背景");
...
void desktop_lrc_init() {
    ...
    gtk_window_move(GTK_WINDOW(desktop_lrc.window), configure.desktop_lrc_x, configure.desktop_lrc_y);
    ...
    gtk_widget_set_colormap(desktop_lrc.window, gdk_screen_get_rgba_colormap(gtk_widget_get_screen(desktop_lrc.window)));
    gtk_widget_realize(desktop_lrc.window);
    gtk_window_set_icon(GTK_WINDOW(desktop_lrc.window),(GdkPixbuf *)gdk_pixbuf_new_from_file("/usr/share/iceplayer/iceplayer.png",NULL));
    ...
    inline void move_or_resize_desktop_lrc(GtkWidget *widget,GdkEventButton *event) {
        if (event->button==3) {
            ...
            GtkWidget *menu_lock = gtk_image_menu_item_new_with_mnemonic("锁定（解锁可以从托盘图标或配置处解锁）");
            gtk_menu_shell_append(GTK_MENU_SHELL(menu),menu_lock);

            GtkWidget *menu_close = gtk_image_menu_item_new_with_mnemonic("关闭（打开可以从托盘图标或配置处打开）");
            gtk_menu_shell_append(GTK_MENU_SHELL(menu),menu_close);

            GtkWidget *menu_jump_to_configure = gtk_image_menu_item_new_with_mnemonic("跳到配置处");
            gtk_menu_shell_append(GTK_MENU_SHELL(menu),menu_jump_to_configure);

            GtkWidget *menu_tips = gtk_image_menu_item_new_with_mnemonic("Tips:移动请按左键，大小请加Ctrl");
            gtk_menu_shell_append(GTK_MENU_SHELL(menu),menu_tips);

            inline void lock() {
                gtk_toggle_button_set_active(configure_widget.desktop_lrc_lock_checkbutton,TRUE);
                update_configure();
            }
            inline void close() {
                gtk_toggle_button_set_active(configure_widget.use_desktop_lrc_checkbutton,FALSE);
                update_configure();
            }
            inline void jump() {
                show();
                ...
                gtk_notebook_set_current_page(configure_widget.lrc_notebook,2);
            }
