...
gchar * get_iceplayer_widget_lrc_back_pic() {
    ...
    if (configure.widget_lrc_Independent_back_image[0]!=NULL) {
...
gboolean widget_lrc_show() {
    ...
    while (n<= lrc_line_number_to_show ) {
        if (configure.widget_lrc_show_type==  WIDGET_LRC_SHOW_TYPE_HPANED)
            cairo_move_to(cr, 15+gtk_paned_get_position(iceplayer.hpaned), iceplayer.main_fixed->allocation.height + a * widget_lrc_word_height+10);
...
gboolean widget_lrc_size_changed() {
    ...
    void gtk_fixed_move_new2(GtkWidget *fixed,GtkWidget *widget,gint x,gint y) {
        if (widget->parent)
...
void widget_lrc_Independent_init() {
    ...
    gtk_window_set_icon(GTK_WINDOW(widget_lrc.window),(GdkPixbuf *)gdk_pixbuf_new_from_file("/usr/share/iceplayer/iceplayer.png",NULL));
...
// gtk_misc_set_alignment (GTK_MISC (widget_lrc.back_image), 0, 0);
    ...
    gtk_label_set_justify(widget_lrc.label,GTK_JUSTIFY_CENTER);
    ...
    gtk_fixed_move_new (GTK_FIXED (widget_lrc.fixed), widget_lrc.close_eventbox,skin.control_close_x,skin.control_close_y);
    gtk_fixed_move_new (GTK_FIXED (widget_lrc.fixed), widget_lrc.min_eventbox, skin.control_min_x, skin.control_min_y);
    gtk_fixed_move_new (GTK_FIXED (widget_lrc.fixed), widget_lrc.max_eventbox,skin.control_max_x, skin.control_max_y);
    gtk_fixed_move_new (GTK_FIXED (widget_lrc.fixed), widget_lrc.above_eventbox, skin.control_above_x, skin.control_above_y);
    ...
    g_signal_connect ((gpointer) widget_lrc.top_eventbox,       "button_press_event",G_CALLBACK (resize_Independent),RESIZE_TOP);
    ...
    g_signal_connect ((gpointer) widget_lrc.buttom_eventbox,       "button_press_event",G_CALLBACK (resize_Independent),RESIZE_BUTTOM);
    ...
    g_signal_connect ((gpointer) widget_lrc.left_eventbox,       "button_press_event",G_CALLBACK (resize_Independent),RESIZE_LEFT);
    ...
    g_signal_connect ((gpointer) widget_lrc.right_eventbox,       "button_press_event",G_CALLBACK (resize_Independent),RESIZE_RIGHT);
    ...
    g_signal_connect ((gpointer) widget_lrc.buttom_left_eventbox,       "button_press_event",G_CALLBACK (resize_Independent),RESIZE_BUTTOM_LEFT);
    ...
    g_signal_connect ((gpointer) widget_lrc.buttom_right_eventbox,       "button_press_event",G_CALLBACK (resize_Independent),RESIZE_BUTTOM_RIGHT);
    ...
    g_signal_connect ((gpointer) widget_lrc.top_left_eventbox,       "button_press_event",G_CALLBACK (resize_Independent),RESIZE_TOP_LEFT);
    ...
    g_signal_connect ((gpointer) widget_lrc.top_right_eventbox,       "button_press_event",G_CALLBACK (resize_Independent),RESIZE_TOP_RIGHT);
...
void widget_lrc_refresh(gboolean force) {
    ...
    case WIDGET_LRC_SHOW_TYPE_HPANED: {
        if (configure.hpaned_pos>configure.width-20) {
            gtk_paned_set_position(iceplayer.hpaned,configure.width-20>0?configure.width-20:0);
