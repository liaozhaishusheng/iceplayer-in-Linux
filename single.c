...
gint switch_focus;
gchar * in_some_wm_that_cant_rec_window_state_event=FALSE;
...
gchar * get_iceplayer_back_pic() {
    ...
    if (configure.back_image[0]!=NULL) {
...
gboolean short_cuts (GtkWidget *widget,GdkEventKey *event,gint type ) {
    ...
    if (event->type == GDK_KEY_PRESS) {
        ...
        case GDK_Delete: {
            if (gtk_notebook_get_current_page(iceplayer.notebook)==SWITCH_LIST)
...
gboolean move (GtkWidget *widget,GdkEventButton *event,gpointer user_data) {
    ...
    inline gboolean move_widget_lrc_window(gint type) {
        static gint width,height;
        gtk_window_get_position(iceplayer.window,&width,&height);
        switch (type) {
        case 0://y 坐标一致，窗口歌词在主窗口的左边
            gtk_window_move(widget_lrc.window,width-configure.widget_lrc_Independent_width,height);
            ...
            if ((((configure.widget_lrc_Independent_height-configure.height)>0)?(configure.widget_lrc_Independent_height-configure.height):(configure.height-configure.widget_lrc_Independent_height))<10)//窗口高度差距在10以下
                gtk_window_resize(widget_lrc.window,configure.widget_lrc_Independent_width,configure.height);

            break;
        case 1://y 坐标一致，窗口歌词在主窗口的右边
            gtk_window_move(widget_lrc.window,width+configure.width,height);
        ...
        case 2://x 坐标一致，窗口歌词在主窗口的下面
            gtk_window_move(widget_lrc.window,width,height- configure.widget_lrc_Independent_height);
            ...
            if ((((configure.widget_lrc_Independent_width-configure.width)>0)?(configure.widget_lrc_Independent_width-configure.width):(configure.width-configure.widget_lrc_Independent_width))<10)//窗口宽度度差距在10以下
                gtk_window_resize(widget_lrc.window,configure.width,configure.widget_lrc_Independent_height);

            break;
        case 3://x 坐标一致，窗口歌词在主窗口的上面
            gtk_window_move(widget_lrc.window,width,height+configure.height);
            configure.widget_lrc_Independent_x=configure.x;
            configure.widget_lrc_Independent_y=configure.y+configure.widget_lrc_Independent_height;
    ...
    if (event->type == GDK_BUTTON_PRESS) {
        if (event->state==GDK_CONTROL_MASK)
            ...
            if (configure.widget_lrc_show_type==WIDGET_LRC_SHOW_TYPE_INDEPENDENT) {
                ...
                if (t==TRUE) {
                    g_timeout_add(100,move_widget_lrc_window,(gpointer *)type);
                }
            }
...
gboolean mini (GtkWidget *widget,GdkEventButton *event,gpointer user_data) {
    ...
    if (event==NULL||event->type == GDK_BUTTON_RELEASE) {
        if (!configure.mini) {
            ...
            gtk_window_move(iceplayer.mini_window,configure.mini_x,configure.mini_y);
        } else {
...
gboolean change_volume(GtkWidget *vscale) {
    ...
    if (core.pipeline)set_volume( gtk_range_get_value(vscale));
    return FALSE;
}
gboolean volume (GtkWidget *widget,GdkEventButton *event,gpointer user_data) {
    ...
    if (event->type == GDK_BUTTON_RELEASE) {
        ...
        if (iceplayer_volume&&GTK_IS_WIDGET(iceplayer_volume))
            ...
            gtk_range_set_value(vscale,configure.volume);
...
void change_widget_lrc_ui(GtkWidget *widget,gint type) {
    ...
    case WIDGET_LRC_SHOW_TYPE_INDEPENDENT: {
        gtk_window_get_position(widget_lrc.window,&configure.widget_lrc_Independent_x,&configure.widget_lrc_Independent_y);
        gtk_window_get_size(widget_lrc.window,&configure.widget_lrc_Independent_width,&configure.widget_lrc_Independent_height);
        gtk_widget_hide(widget_lrc.window);
    ...
    case WIDGET_LRC_SHOW_TYPE_INTEGRATION: {
        if (widget)gtk_notebook_set_current_page(iceplayer.notebook,SWITCH_LIST);
        break ;
    }
    }
    ...
    case WIDGET_LRC_SHOW_TYPE_INDEPENDENT: {
        if (widget&&configure.show_page==SWITCH_LRC)
            gtk_notebook_set_current_page(iceplayer.notebook,SWITCH_LIST);
        gtk_widget_show_all(widget_lrc.window);
        gtk_window_move(widget_lrc.window,configure.widget_lrc_Independent_x,configure.widget_lrc_Independent_y);
    ...
    case WIDGET_LRC_SHOW_TYPE_INTEGRATION: {
        if (widget) {
            gtk_notebook_set_current_page(iceplayer.notebook,SWITCH_LRC);
    ...
    case WIDGET_LRC_SHOW_TYPE_HPANED: {
        ...
        gtk_label_set_justify(iceplayer.widget_lrc_label_hpaned,GTK_JUSTIFY_CENTER);
        ...
        if (event->button==1) {
            if (type==SWITCH_LRC) {
                ...
                case WIDGET_LRC_SHOW_TYPE_HPANED: {
                    ...
                    type =  SWITCH_LIST;
                    configure.show_page=SWITCH_LIST;
                    gtk_notebook_set_current_page(iceplayer.notebook,type);
                    type=SWITCH_LRC;
                    return TRUE;
                }
                case WIDGET_LRC_SHOW_TYPE_INTEGRATION: {
...
gboolean main_window_menu() {
    ...
    GtkWidget *equalizer_item;
    GtkWidget *mini_item;
    GtkWidget *separator_4;
    GtkWidget *lrc_item;
    GtkWidget *lrc_item_menu;
    GtkWidget *switch_to_lrc_iem;
    GtkWidget *show_desktop_lrc_item;
    GtkWidget *search_lrc_with_custom_word_item;
    GtkWidget *separator_5;
    GtkWidget *lock_desktop_lrc_item;
    GtkWidget *edit_lrc_item;
    ...
    GtkWidget *play_mode_item;
    GtkWidget *play_mode_item_menu;
    GtkWidget *play_mode_normal_item;
    GtkWidget *play_mode_unnormal_item;
    GtkWidget *play_mode_radom_item;
    GtkWidget *play_mode_one_item;
    GtkWidget *separator_7;
    GtkWidget *separator_8;
    GtkWidget *play_mode_other_lists_item;
    GtkWidget *lists_add_files;
    GtkWidget *lists_add_folder;
    GtkWidget *show_lrc_item;
    GtkWidget *menu_research_lrc;
    GtkWidget *menu_research;
    GtkWidget *menu_INDEPENDENT;
    GtkWidget *menu_HPANED;
    GtkWidget *menu_INTEGRATION;
    ...
    inline void jump_configure() {
        show();
        gtk_notebook_set_current_page(iceplayer.notebook,SWITCH_CONFIG);
        gtk_notebook_set_current_page(configure_widget.notebook,CONFIG_SW_NORMAL);
    ...
    inline void check_update() { //注意要使用多线程检测更新
        g_thread_create_full((GThreadFunc)auto_Update, TRUE,0,FALSE,TRUE,G_THREAD_PRIORITY_LOW,NULL);
    }
    g_signal_connect((gpointer) menu_update ,"activate" ,G_CALLBACK(check_update),NULL);
    inline void go_to_iceplayer_forum() { //注意要使用多线程检测更新
        gtk_show_uri(NULL,"http://www.iceplayer.org",NULL,NULL);
    }
    g_signal_connect((gpointer) menu_web ,"activate" ,G_CALLBACK(go_to_iceplayer_forum),NULL);
    inline void jump_help() {
        show();
        gtk_notebook_set_current_page(iceplayer.notebook,SWITCH_HELP);
        configure.show_page=SWITCH_HELP;
    }
    g_signal_connect((gpointer) menu_help ,"activate" ,G_CALLBACK(jump_help),NULL);
    g_signal_connect((gpointer) menu_about ,"activate" ,G_CALLBACK(about),NULL);
...
*/
//跳到正在播放歌曲
gboolean  list_menu_jump_to_playing_song() {
...
void list_menu_rename() {
    ...
    while (1) {
        ...
        gtk_window_resize(input_dialog,500,50);
        ...
        gint a = gtk_dialog_run(input_dialog);

        if (a==-5&&gtk_entry_get_text(entry)!=NULL) {
            if (strcmp(gtk_entry_get_text(entry),"")&&strcmp(gtk_entry_get_text(entry),entry_org)) {
                strcpy(entry_text,gtk_entry_get_text(entry));
                strcpy(entry_title,rename_songs(value,gtk_entry_get_text(entry)));
...
void list_menu_switch_to_lrc (GtkMenuItem *menuitem,gpointer user_data) {
    show();
    gtk_notebook_set_current_page(iceplayer.notebook,SWITCH_CONFIG);
    gtk_notebook_set_current_page(configure_widget.notebook,CONFIG_SW_LRC);
    gtk_notebook_set_current_page(configure_widget.lrc_notebook,0);
}
...
///*冒泡排序*/
    ...
    show_songs(NULL);
    if (strcmp(focus_lists,playing_list)==0)list_menu_jump_to_playing_song();
}
...
void show_list_menu () {
    ...
    GtkWidget *play_selection_item;
    GtkWidget *deal_with_playing_song_item;
    GtkWidget *deal_with_playing_song_item_menu;
    GtkWidget *open_dir_of_song_item;
    GtkWidget *man_tools_item;
    GtkWidget *separator_2;
    GtkWidget *rename_item;
    GtkWidget *input_lrc_item;
    GtkWidget *open_with_audacious_item;
    GtkWidget *open_with_amarok_item;
    GtkWidget *open_with_totem_item;
    GtkWidget *delete_item;
    GtkWidget *delete_item_menu;
    GtkWidget *deletet_selection_item;
    GtkWidget *deletet_same_item;
    GtkWidget *delete_focus_lists;
    GtkWidget *separator_3;
    GtkWidget *delete_no_exist_item;
    GtkWidget *real_delete_item;
    GtkWidget *id3_item;
    GtkWidget *equalizer_item;
    GtkWidget *separator_4;
    GtkWidget *lrc_item;
    ...
    GtkWidget *search_item_menu;
    GtkWidget *search_local_song_item;
    GtkWidget *search_music_box_item;
    GtkWidget *lists_add_files;
    GtkWidget *lists_add_folder;
    GtkWidget *show_lrc_item;
...
void show_lists_menu() {
    ...
    inline void rename_l() {
        ...
        gint a = gtk_dialog_run(input_dialog);
        if (a==-5&&gtk_entry_get_text(entry)!=NULL) {
            if (strcmp(gtk_entry_get_text(entry),""))rename_lists(value,gtk_entry_get_text(entry));
        }
        gtk_widget_destroy(input_dialog);
        if (value)g_free_n(value);
    }
