...
void play_uri() {
    ...
    if (strncmp(focus_song,"<cue>",5)) {
        ...
        gtk_label_set_text( iceplayer.name_label,g_basename(path));
        print_notify(_("播放"),g_basename(path));
    } else {
        ...
        gtk_label_set_text( iceplayer.name_label,strcat(strcat(title,"-"),performer));
