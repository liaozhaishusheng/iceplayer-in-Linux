...
void ui_init() {
    ...
    gtk_window_set_icon(GTK_WINDOW(iceplayer.window),(GdkPixbuf *)gdk_pixbuf_new_from_file("/usr/share/iceplayer/iceplayer.png",NULL));
    iceplayer.trayIcon=gtk_status_icon_new_from_file("/usr/share/iceplayer/iceplayer_tray.png");
...
///songs
    ...
    gtk_tree_view_set_enable_tree_lines(iceplayer.list_songs_treeview,FALSE);
    ...
    gtk_tree_view_set_enable_tree_lines(iceplayer.list_lists_treeview,FALSE);
    ...
    gtk_target_list_add_uri_targets(list,dest_targets);
    gtk_drag_dest_set_target_list( GTK_WIDGET (iceplayer.list_songs_treeview),list);

///switch > lrc
    iceplayer.widget_lrc_label = gtk_label_new ("");
    gtk_label_set_justify(iceplayer.widget_lrc_label,GTK_JUSTIFY_CENTER);
...
///resize
    ...
    inline gboolean change_current_page() {
        configure.show_page = gtk_notebook_get_current_page(iceplayer.notebook);
        return FALSE;
    }
...
///progress
    ...
    g_signal_connect ((gpointer) iceplayer.progress_left_eventbox, "scroll-event", progress_scroll, NULL);
    ...
    g_signal_connect ((gpointer) iceplayer.progress_node_eventbox, "scroll-event", progress_scroll, NULL);
    ...
    g_signal_connect ((gpointer) iceplayer.progress_right_eventbox, "scroll-event", progress_scroll, NULL);
    ...
    g_signal_connect ((gpointer) iceplayer.volume_eventbox,     "scroll-event", volume_scroll, NULL);
///list:
...
gchar *get_skin_dir() {
    ...
    if (skin.use_default_skin) {
        strcpy(dir,"/usr/share/iceplayer/skin/blue_life_slax/");
    } else {
        if (use_usr_skin) {
            strcpy(dir,"/usr/share/iceplayer/skin/");
...
void set_skin_configure_to_iceplayer_widget() {
    ...
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.menu_eventbox, skin.control_menu_x, skin.control_menu_y);
    gtk_widget_set_size_request (iceplayer.menu_eventbox, skin.control_menu_width, skin.control_menu_height);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.above_eventbox, skin.control_above_x, skin.control_above_y);
    gtk_widget_set_size_request (iceplayer.above_eventbox,skin.control_above_width , skin.control_above_height);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.min_eventbox, skin.control_min_x, skin.control_min_y);
    gtk_widget_set_size_request (iceplayer.min_eventbox, skin.control_min_width, skin.control_min_height);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.max_eventbox, skin.control_max_x,  skin.control_max_y);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.mini_eventbox, skin.control_mini_x, skin.control_mini_y);
    ...
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.close_eventbox,  skin.control_close_x, skin.control_close_y);
    gtk_widget_set_size_request (iceplayer.close_eventbox,skin.control_close_width,skin.control_close_height);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.prev_eventbox, skin.control_prev_x, skin.control_prev_y);
    gtk_widget_set_size_request (iceplayer.prev_eventbox, skin.control_prev_width, skin.control_prev_height);

    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.play_eventbox, skin.control_play_x,skin.control_play_y);
    gtk_widget_set_size_request (iceplayer.play_eventbox, skin.control_play_width,skin.control_play_height);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.next_eventbox, skin.control_next_x, skin.control_next_y);
    gtk_widget_set_size_request (iceplayer.next_eventbox, skin.control_next_width, skin.control_next_height);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.stop_eventbox, skin.control_stop_x, skin.control_stop_y);
    gtk_widget_set_size_request (iceplayer.stop_eventbox, skin.control_stop_width, skin.control_stop_height);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.open_eventbox, skin.control_open_x, skin.control_open_y);
    gtk_widget_set_size_request (iceplayer.open_eventbox, skin.control_open_width, skin.control_open_height);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.progress_hbox, skin.progress_left_top_x, skin.progress_left_top_y);
    gtk_fixed_move_new (GTK_FIXED (iceplayer.main_fixed), iceplayer.volume_eventbox, skin.volume_x, skin.volume_y);
    ...
    sscanf(gtk_entry_get_text(configure_widget.info_name_entry_x),"%d",&info_name_x);
    sscanf(gtk_entry_get_text(configure_widget.info_name_entry_y),"%d",&info_name_y);

    sscanf(gtk_entry_get_text(configure_widget.info_time_entry_x),"%d",&info_time_x);
    sscanf(gtk_entry_get_text(configure_widget.info_time_entry_y),"%d",&info_time_y);

    if(!(info_name_x==0&&info_name_y==0)) {
        gtk_fixed_move_new(GTK_FIXED (iceplayer.main_fixed),iceplayer.name_label,info_name_x,info_name_y);
    } else {
    ...
    if(!(info_time_x==0&&info_time_y==0)) {
        gtk_fixed_move_new(GTK_FIXED (iceplayer.main_fixed),iceplayer.time_label,info_time_x,info_time_y);
    } else {
...
gboolean progress_refresh() {
    ...
    if (core.state==GST_STATE_PLAYING||core.state ==GST_STATE_PAUSED ) {
        ...
        if (!strncmp(focus_song,"<cue>",5)) {
            ...
            gtk_label_set_text( iceplayer.time_label,A);
        } else {
...
void tray_menu() {
    ...
    g_signal_connect ((gpointer) tray_about, "activate",G_CALLBACK (about),NULL);
